...
DATE=$(date +%y-%m-%d-%H-%M)
RESULT_DIRECTORY="/users/jfrau/am/results/IBM/IBM_solucionar_problemas/4timesmoreSentencesNoClaim_than_Iclaim"
...
DATASET_NAME="/users/jfrau/am/IBMdataset/IBM_solucionar_problemas/4timesmoreSentencesNoClaim_than_Iclaim/4timesmoreSentencesNoClaim_than_Iclaim_komninos_e.p "
# The model resulting from this command will be saved in --output_dirpath + --experiment_name _model.h5
python3 -u experiments/train_BiLSTM_CNN_CRF.py \
    --dataset $DATASET_NAME \
    --attention_model $ATTENTION_MODEL \
    --attention_activation $ATTENTION_ACTIVATION \
    --output_dirpath $RESULT_DIRECTORY/$EXPERIMENT_DIRECTORY \
    --experiment_name 4timesmoreNoClaim \
    --char_embedding $1 \
...
MODEL_NAME=$(compgen -f $RESULT_DIRECTORY/$EXPERIMENT_DIRECTORY/*.h5)
